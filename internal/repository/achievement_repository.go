...
func (r *achievementRepository) AssignAchievementIfEligible(userId uint) ([]model.HaveAchievement, error) {
	...
	err := r.db.
			Table("have_badges").
			Select("badge_id, MIN(created_at) as created_at").
	...
	SELECT 
			...
			COUNT(DISTINCT tq.id) AS total_quests
	FROM unique_dates u
	LEFT JOIN take_lessons tl ON DATE(tl.created_at) = u.take_date AND tl.user_id = ?
	LEFT JOIN take_exercises te ON DATE(te.created_at) = u.take_date AND te.user_id = ?
	LEFT JOIN take_quests tq ON DATE(tq.created_at) = u.take_date AND tq.user_id = ?
	GROUP BY u.take_date
	ORDER BY u.take_date ASC;

	`

	err = r.db.Raw(queryCase2, userId, userId, userId, userId, userId, userId).Scan(&totalTakesPerDay).Error
